    ...
    <div className="customer-shell">
      <header className="customer-topbar">
        ...
        <div className="customer-topbar-actions">
          {/* Desktop-only: a real quick-action, not a duplicate of the nav
           * row below it — the bottom/secondary nav already covers Cart on
           * every screen size, this is just a shortcut so a desktop visitor
           * doesn't have to look elsewhere. Hidden on mobile via CSS, where
           * the bottom nav's own Cart tab is already prominent. */}
          <NavLink to="/customer/cart" className="customer-topbar-cart" aria-label={cartCount > 0 ? `View cart, ${cartCount} item${cartCount === 1 ? '' : 's'}` : 'View cart'}>
            <ShoppingCart size={19} strokeWidth={1.8} />
            {cartCount > 0 && <span className="customer-cart-badge">{cartCount}</span>}
          </NavLink>

          {customer
            ? <NavLink to="/customer/profile" className="customer-topbar-user">
                <User size={14} strokeWidth={2} />
                Hi, {customer.name.split(' ')[0]}
              </NavLink>
            : <NavLink to="/customer/login" className="customer-topbar-user customer-topbar-signin">Sign in</NavLink>}
        </div>
      </header>
