...
export default function SalesPage() {
  ...
  useEffect(() => {
    ...
    // Products for the Sale page's picker (ProductSearchCell) and manual
    // search: online, exactly as before (GET /products, capped at 500) —
    // completely unchanged. Offline (page opened with no connection, or
    // the request fails mid-flight), or the moment connectivity comes
    // back, fall back to / refresh from the same IndexedDB catalog cache
    // catalogSync.ts already keeps populated — no mock data, no new
    // store, just the existing offline product cache mapped to the same
    // `Product` shape the online list already uses.
    if (!isOnline) {
      if (companyId) {
        getAllProductsOffline(companyId).then(rows => setProducts(rows.map(toOfflineProduct))).catch(() => {})
      }
      return
    }
    productsAPI.list({ limit: 500 }).then(r => setProducts(r.data.data || [])).catch(async (err: any) => {
      if (isNetworkError(err) && companyId) {
        try {
          const rows = await getAllProductsOffline(companyId)
          setProducts(rows.map(toOfflineProduct))
        } catch { /* leave products as-is */ }
      }
    })
  }, [isOnline, companyId])
